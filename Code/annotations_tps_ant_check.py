def get_dict_annot_ant(annot_path, filename):
	lines = []
	with open(annot_path + filename) as f:
		lines = f.readlines()

	# {img_name : [[[landmarks1], [landmarks2], [landmarks3]], lm, scale]}
	coordinates = {}

	# [[landmarks1], [landmarks2], [landmarks3]]
	landmarks = [[], [], []]
	pos = 0
...
if __name__ == '__main__':
	...
	for value in coordinates.items():
		...
		if len(value[1][0][0]) != 24 or len(value[1][0][1]) != 10 or len(value[1][0][2]) != 10:
			print(value[0])
